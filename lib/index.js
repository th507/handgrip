...
function registerGeneratorHelper(fnName, fn) {
  ...
  return hbs.registerHelper(fnName, function() {
    ...
    if (Q in root) {
      // be compatible with some generator helpers
      // which use Object.create instead of createFrame
      // to create a child scope or data object
...
function createRenderer(compiled) {
  return function*(context) {
    ...
    var result = yield replace(context[Q], out, context);

    delete context.spliceOffset;
    delete context[Q];
    return result;
  };
}
...
function createFrame(obj) {
  ...
  if (frame.root) {
    frame.root = Object.create(frame.root);
    frame.root.spliceOffset = 0;
  }
  return frame;
}
