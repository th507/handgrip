...
function registerGeneratorHelper(fnName, fn) {
  var hbs = this
  // compatible with default registerHelper
  ...
  return hbs.registerHelper(fnName, createGeneratorHelper(fn))
}

/**
 * Handler factory for generator helper
 **/
function createGeneratorHelper(fn) {
  return function generatorHelperHandler() {
...
/**
 * Find root scope for generator helper list insertion
 * @param arr
 **/
function findRoot(arr) {
  var options = arr[arr.length - 1]
...
function createRenderer(compiled) {
  return function* HandgripGeneratorRenderer(context_) {
    // avoid memory leak by cloning arguments
    var len = arguments.length
    var args = Array(len)
    while (len-- > 0) args[len] = arguments[len]
